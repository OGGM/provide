...
resolution_dir = 'total_data' #'resolution_1_deg'

preprocess_region_dict_outpath = os.path.join(base_dir, 'glacier_regions', resolution_dir)
utils.mkdir(preprocess_region_dict_outpath);

grid_plots_outpath = os.path.join(base_dir, 'glacier_regions', resolution_dir, 'grid_plots')
utils.mkdir(grid_plots_outpath);

aggregated_data_outpath = os.path.join(base_dir, 'glacier_regions', resolution_dir, 'aggregated_data')
...
with open(os.path.join(regions_data_dir,
                       "dict_regions_to_rgi_ids.json"), "r") as f:
    dict_regions_to_rgi_ids = json.load(f)
...
if is_notebook:
    test_region_index = gdf_regions[gdf_regions[name_col_regions] == 'new_zealand'].index[0]
    test_region = gdf_regions.iloc[test_region_index: test_region_index + 1]

# ## get raw grid for country

if is_notebook:
    test_ds_global_grid = get_global_grid(resolution)
    test_ds_local = get_squared_grid_for_shape(test_ds_global_grid, test_region)
    ...
    test_region.plot(ax=ax, color='blue', alpha=0.5)

# ## assign rgi_ids to grid points

if is_notebook:
    assign_rgi_ids_to_grid_points(test_ds_local,
                                  test_region[name_col_regions].values.item(),
                                  dict_regions_to_rgi_ids,
                                  gdf_rgi_ids)
    plot_local_grid_with_glaciers(gdf_rgi_ids,
                                  test_region, test_ds_local,
                                  title=f'{test_region[name_col_regions].values[0]}, resolution '
...
if is_notebook:
    ...
    test_key = test_region[name_col_regions].values[0]
    final_dict = {test_key: test_ds_local_dict}

    if save_sample_data:
        with open("prepocessed_region_grids_sample_data.json", "w") as outfile: 
...
if not is_notebook:
    ...
    final_dict = {}
    regions_with_no_glaciers = []

    ds_global_grid_resolution = get_global_grid(resolution)

    # loop through all regions and add local grid to final dict
    nr_of_regions = len(gdf_regions)
    for index, row in gdf_regions.iterrows():
        print(f'Starting region number {index + 1} of {nr_of_regions}.')
        region = gpd.GeoDataFrame([row], crs=gdf_regions.crs)

        ds_local_grid = get_squared_grid_for_shape(ds_global_grid_resolution,
                                                   region)

        for grid in ds_local_grid:
            grid.attrs['region'] = region[name_col_regions].values[0]
        assign_rgi_ids_to_grid_points(ds_local_grid,
                                      region[name_col_regions].values.item(),
                                      dict_regions_to_rgi_ids,
                                      gdf_rgi_ids)
        if save_local_grid_plots:
            plot_local_grid_with_glaciers(
                gdf_rgi_ids, region, ds_local_grid,
                title=f'{region[name_col_regions].values[0]}, '
                f'{ds_local_grid[0].resolution} °',
                outpath=os.path.join(grid_plots_outpath,
                                     f'{region[name_col_regions].values[0]}.png'))
        ...
        if ds_local_grid.grid_points_with_data == 0:
            ...
            regions_with_no_glaciers.append(region[name_col_regions].values[0])
        else:
            # we have some glaciers so let's add
            ds_local_grid_dict = serialize_dataset(ds_local_grid).to_dict()
            final_dict[region[name_col_regions].values[0]] = ds_local_grid_dict

        print(f'Finished region number {index + 1} of {nr_of_regions}.')

    # save final preprocessed file in input dir
    with open(os.path.join(preprocess_region_dict_outpath,
                           "preprocessed_region_grids.json"), "w") as outfile: 
        json.dump(final_dict, outfile)

    # save list of countries with no glaciers
    with open(os.path.join(aggregated_data_outpath,
                           "regions_with_no_glaciers.json"), "w") as outfile: 
        json.dump(regions_with_no_glaciers, outfile)
